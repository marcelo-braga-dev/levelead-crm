...
| Termo | Definição |
|---|---|
| **Company** | Entidade cadastral do Lead — **PF ou PJ desde 2026-06-22** (`person_type` enum), antes PJ-only. PJ usa `cnpj`/`razao_social`/`nome_fantasia` + satélites (sócios, CNAE, faturamento, dívidas); PF usa só `cpf` + `razao_social` (rótulo de UI muda para "Nome completo", mesma coluna — não há coluna `nome` separada). Independente do funil de vendas — uma Company pode existir sem nenhum Lead, ou ter tido vários Leads ao longo do tempo. `Company`/`companies` continuam sendo o nome do model/tabela/rotas (`companies.*`) — só a **terminologia de frontend** (nav, títulos, textos de tela) usa "Lead"/"Leads" em vez de "Empresa"/"Empresas", para não insinuar que todo Lead é PJ. Ver `PLANO_IMPLEMENTACAO.md` seção "Lead como entidade completa (PF/PJ unificado)". |
| **Lead** | Entidade do funil de vendas (Kanban). Sempre vinculado a uma `Company` via `company_id`. Tem campos de contato de trabalho próprios (copiados da Company na criação) e de qualificação (`interest_level`/`purchase_potential`/`qualification_notes`) — **editáveis só pela página Leads** (`companies.update` para admin/manager, `leads.contact.update` para o consultor responsável pelo próprio lead), nunca pelo Kanban, que é só para mover/trabalhar o lead (interações, propostas, acompanhamentos, endereço). |
